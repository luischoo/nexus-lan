...
function AmbientStars() {
  ...
  useEffect(() => {
    const onScroll = () => setScroll(window.scrollY / Math.max(1, document.body.scrollHeight - window.innerHeight))
  ...
  useFrame(({ clock, pointer }) => {
    ...
    camera.position.z = 8 - scroll * 1.1
  })
...
export function UniverseCanvas() {
  return <div className="universe-canvas" aria-hidden="true"><Canvas camera={{ position: [0, 0, 8], fov: 60 }} dpr={[1, 1.5]}><ambientLight intensity={0.3} /><AmbientStars /></Canvas></div>
}

function LogoParticles({ dispersed }: { dispersed: boolean }) {
  const pointsRef = useRef<THREE.Points>(null)
  ...
  useFrame(({ clock, pointer }) => {
    ...
    targetRotation.current.y = dragging ? targetRotation.current.y : pointer.x * 0.22
...
export function NexusParticleLogo() {
  ...
  return <div ref={sectionRef} className="nexus-particle-stage"><div className="particle-stage-label"><span className="live-dot" /> INTERACTIVE PARTICLE TOPOLOGY <small>{dispersed ? "DISPERSED" : "FORMED"}</small></div><Canvas camera={{ position: [0, 0, 10], fov: 42 }} dpr={[1, 1.5]}><LogoParticles dispersed={dispersed} /></Canvas><p>HOVER / DRAG TO ROTATE · SCROLL TO DISPERSE</p></div>
}
